...
async function decodeQrFromWebpBuffer(buffer) {
  try {
    ...
    try {
      ...
      console.log('[antilink-warn] Sticker non binarizzabile come QR, salto il controllo.', qrErr.message)
      return null
    }
  } catch (err) {
    console.log('[antilink-warn] Errore lettura WebP:', err.message)
    return null
  }
}
...
export async function before(m, { conn, isAdmin, isBotAdmin }) {
  ...
  if (linkRegex.test(text)) {
    ...
    if (!thisGroupCode) {
      try {
        ...
        console.log('[antilink-warn] Errore recupero invite:', e)
        return true
      }
    }
    ...
    try {
      ...
      console.error('[antilink-warn] Errore durante espulsione:', e)
    }
  ...
  async function handleQrMedia(m, buffer, isSticker = false) {
    ...
    if (isSticker || isWebP(buffer)) {
      try {
        ...
        console.log('[antilink-warn] Errore lettura sticker QR WebP:', err.message)
      }
    }
    ...
    if (!qrText) {
      try {
        ...
        console.log('[antilink-warn] Errore lettura QR con canvas:', e.message)
      }
    }
    ...
    if (!thisGroupCode) {
      try {
        ...
        console.log('[antilink-warn] Errore invite QR:', e)
        return true
      }
    }
    ...
    try {
      ...
      console.error('[antilink-warn] Errore espulsione QR:', e)
    }
  ...
  if (m.mtype === 'imageMessage' || m.mtype === 'stickerMessage') {
    try {
      ...
      console.log('[antilink-warn] Errore download QR:', e.message)
    }
  }
