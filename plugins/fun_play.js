...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (command === "play") {
    ...
    // Salva la scelta dell'utente in memoria temporanea
    ...
    let infoMsg = `┏━━━━━━━━━━━━━━━━━━━┓\n` +
                  ...
                  `*Rispondi a questo messaggio scrivendo:* \n` +
                  `👉 *${usedPrefix}playaud* per l'Audio (MP3)\n` +
                  `👉 *${usedPrefix}playvid* per il Video (MP4)`

    // Invia immagine con istruzioni testuali stabili al 100% su ogni dispositivo
  ...
  if (command === "playaud") {
    ...
    if (!downloadUrl) {
      return m.reply('🚀 *走𝙡𝙖𝙮 𝙀𝙧𝙧𝙤𝙧:* Al momento i server di download sono sovraccarichi o offline. Riprova tra poco.')
    }
