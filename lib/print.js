...
function formatType(m) {
  ...
  let mtype = m.mtype || 'unknown';

  if (VIEW_ONCE_TYPES.includes(mtype)) {
    const innerMsg = m.message?.[mtype]?.message || {};
    const innerType = Object.keys(innerMsg).find(k => !['messageContextInfo', 'senderKeyDistributionMessage'].includes(k));
    if (innerType) {
      mtype = innerType;
    }
  }
  ...
  const labels = {
    imageMessage: '📷 Foto (1 Vis)',
    videoMessage: '🎥 Video (1 Vis)',
    audioMessage: '🎵 Audio (1 Vis)',
    image: '📷 Foto',
    video: '🎥 Video',
    audio: '🎵 Audio',
    stickerMessage: '🌟 Sticker',
    documentMessage: '📄 Documento',
    contactMessage: '👤 Contatto',
    locationMessage: '📍 Posizione',
    liveLocationMessage: '📡 Posizione Live',
    productMessage: '🛍️ Prodotto',
    pollCreationMessage: '🗳️ Sondaggio',
    reactionMessage: '✧ Reazione',
    protocolMessage: '🔒 Protocollo',
    conversation: '💬 Testo',
    extendedTextMessage: '💬 Testo',
    ephemeralMessage: '⏳ Messaggio Effimero',
    ptvMessage: '🎥 PTV (1 Vis)'
  };

  const cleanType = mtype.replace(/Message$/i, '');
  return labels[mtype] || labels[cleanType] || cleanType;
}

function getViewOnceInner(m) {
  const VIEW_ONCE_TYPES = ['viewOnceMessage', 'viewOnceMessageV2', 'viewOnceMessageV2Extension'];
  if (!VIEW_ONCE_TYPES.includes(m?.mtype)) return null;
  return m.message?.[m.mtype]?.message || null;
}
...
function logMessageSpecifics(m, c) {
  const VIEW_ONCE_TYPES = ['viewOnceMessage', 'viewOnceMessageV2', 'viewOnceMessageV2Extension'];
  const isViewOnce = VIEW_ONCE_TYPES.includes(m?.mtype);

  let innerMsg = null;
  if (isViewOnce) {
    innerMsg = m.message?.[m.mtype]?.message || null;
    m = {
      ...m,
      msg: innerMsg ? Object.values(innerMsg)[0] : m.msg,
      mtype: innerMsg ? Object.keys(innerMsg)[0] : m.mtype
    };
  }

  const specifics = {
    viewOnceMessage: () => {
      const label = formatType(m);
      console.log(`${c.secondary('👁️')} ${label}`);
    },
    viewOnceMessageV2: () => {
      const label = formatType(m);
      console.log(`${c.secondary('👁️')} ${label}`);
    ...
    ptvMessage: () => {
      ...
      console.log(`${c.secondary('🎥')} PTV (Video 1 Vis)${duration ? ' - ' + duration : ''}`);
    },
    ephemeralMessage: () => {
      const inner = m.message?.ephemeralMessage?.message || null;
      const innerType = inner ? Object.keys(inner).find(k => !['messageContextInfo', 'senderKeyDistributionMessage'].includes(k)) : null;
      console.log(`${c.secondary('⏳')} Messaggio Effimero${innerType ? ` (${innerType.replace(/Message$/i, '')})` : ''}`);
    },
    documentMessage: () => {
